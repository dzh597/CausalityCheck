...
- **Sole Modification**: You may only modify the `context` field by inserting a sentence. The JSON structure and all other fields (`id`, `question`, `choices`, `answerable`) must remain **unchanged**.
...
def run(*, model_name: str, input_file: str, output_file: str, api_key: str, api_base: str):
